...
void loop() {
    ...
    if (receivedAck) {
        ...
        if (msgId == POLL_ACK) {
            ...
            transmitPoll();
            noteActivity();
        } else if (msgId == COLOR_RESPONSE) {
            expectedMsgId = POLL_ACK;
            // neopixel update
            memcpy(&rgb.red, data + 1, 1);
            memcpy(&rgb.blue, data + 2, 1);
            memcpy(&rgb.green, data + 3, 1);
            memcpy(&rgb.white, data + 4, 1);

            for (int i = 1; i < NUMPIXELS; i++) {
              pixels.setPixelColor(i, pixels.Color(rgb.green, rgb.white, rgb.blue, rgb.red));
            } 
            pixels.show(); 
            transmitPoll();
            noteActivity();
